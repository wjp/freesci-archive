...
#ifdef ARM_WINCE
#  define TITLE_STRING "FreeSCI for PocketPC, version "VERSION" "
#else
#  define TITLE_STRING "FreeSCI, version "VERSION" "
#endif
...
gfx_box_text(gfx_driver_t *gfx_driver, rect_t box, gfx_bitmap_font_t* font, gfx_pixmap_color_t fgc, gfx_pixmap_color_t bgc, char* text, int center)
{
	...
	if (center != 0)
	{
		textsplits = gfxr_font_calculate_size(font, box.xl, text, &meas_width, &meas_height, &meas_line, GFXR_FONT_FLAG_NO_NEWLINES | GFXR_FONT_FLAG_COUNT_WHITESPACE);
		if (textsplits != NULL)
		{
			free(textsplits);
		}
