...
void *
_xrealloc (void *oldp, size_t size)
{
...
con_init (void)
{
  ...
  {
    _cmd_commands =
      _xrealloc (NULL, sizeof (cmd_command_t) * (_cmd_command_mem = 32));
    atexit (_cmd_exit);
...
sciprintf (char *fmt, ...)
{
  ...
  while (i > 0)
  {
    ...
    int seeker =
      seekerpt ? seekerpt - mbuf : SCI_CONSOLE_LINE_WIDTH -
      con_outputbufcolumn;
    ...
    memcpy (con_outputbuf[con_outputbufpos] + con_outputbufcolumn, mbuf,
	    seeker);
