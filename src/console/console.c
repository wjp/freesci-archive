...
drawConsole(picture_t pic)
{
  if (con_visible_rows > 0) {
    ...
    while (blen--) { /* Draw console background */
      guint8 pixel = pic[0][blen] &= (SCI_CONSOLE_BGCOLOR | SCI_CONSOLE_BGCOLOR << 4);
      //      pic[0][blen] = pixel | ((pixel + SCI_CONSOLE_BGCOLOR) & 0x1) << 4;
    }
    memset(&(pic[0][((con_visible_rows << 3) + 1)*320]), SCI_CONSOLE_BORDERCOLOR, 320);
    /* Draw border line */
    ...
    if (pos >= 0) /* Draw the cursor */
      for (i=0; i < 7; i++)
	memset(&(pic[0][pos += 320]), SCI_CONSOLE_CURSORCOLOR, 7);
	...
	nextline += SCI_CONSOLE_OUTPUT_BUFFER;
      ...
      drawString(pic, pos, SCI_CONSOLE_LINE_WIDTH,
		 _outputbuf[nextline], SCI_CONSOLE_FGCOLOR);
    }
    ...
    while (inplines--) {
      drawString(pic, pos++, SCI_CONSOLE_LINE_WIDTH,
		 _inpbuf + i, SCI_CONSOLE_INPUTCOLOR);
