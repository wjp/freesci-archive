...
void make_sbi(adlib_def *one, adlib_def *two, guint8 *buffer)
{
  ...
  buffer[0] |= ((one->envelope & 0x1) << 5);
  buffer[0] |= ((one->keybdscale & 0x1) << 4);
  buffer[0] |= (one->freqmod & 0xf);
  buffer[1] |= ((two->ampmod & 0x1) << 7);
  buffer[1] |= ((two->vibrato & 0x1) << 6);
  buffer[1] |= ((two->envelope & 0x1) << 5);
  buffer[1] |= ((two->keybdscale & 0x1) << 4);
  buffer[1] |= (two->freqmod & 0xf);
  buffer[2] |= ((one->keyscale & 0x3) << 6);
  buffer[2] |= (one->volume & 0x3f);
  buffer[3] |= ((two->keyscale & 0x3) << 6); 
  buffer[3] |= (two->volume & 0x3f);
  buffer[4] |= ((one->attackrate & 0xf) << 4);
  buffer[4] |= (one->decayrate & 0xf);  
  buffer[5] |= ((two->attackrate & 0xf) << 4);
  buffer[5] |= (two->decayrate & 0xf);  
  buffer[6] |= ((one->sustainvol & 0xf) << 4);
  buffer[6] |= (one->releaserate & 0xf);
  buffer[7] |= ((two->sustainvol & 0xf) << 4);
